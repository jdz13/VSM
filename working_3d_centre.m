...
no = 2; % Parameter to determine whether even/odd no of cells. Even = even. 

extra = 10^-10;

linex = (-rangex:2*rangex/(2*nx+no-1):rangex)+extra;
liney = (-rangey:2*rangey/(2*ny+no-1):rangey)+extra;
linez = (-rangez:2*rangez/(2*nz+no-1):rangez)+extra;
...
Mag = zeros(2*nx+no,2*ny+no, 2*nz+no);

% Add in the dirac delta function somewhere

Mag(floor(size(Mag,1)/2),floor(size(Mag,2)/2),floor(size(Mag,3)/2)) = 1;
Mag(floor(size(Mag,1)/2+1),floor(size(Mag,2)/2),floor(size(Mag,3)/2)) = -1;
...
quivdatX = Xcompfil(:,:,size(real_part,1)/2+1);
%quivdatX = reshape(quivdatX, size(X,2), size(X,3));
quivdatZ = Zcompfil(:,:,size(real_part,3)/2+1);
%quivdatZ = reshape(quivdatZ, size(Z,2), size(Z,3));
quivdatY = Ycompfil(:,:,size(real_part,2)/2+1);

quivx = X(:,:,size(real_part,2)/2);
quivz = Z(:,:,size(real_part,2)/2);
quivy = Y(:,:,size(real_part,2)/2);
